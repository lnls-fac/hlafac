...
def set_reforbit():
    ...
    _reforbit_x = _np.copy(_api_status.get_reforbit('x'))[_api_status.get_device_idx('bpm_x')]
    _reforbit_y = _np.copy(_api_status.get_reforbit('y'))[_api_status.get_device_idx('bpm_y')]
...
def set_respm():
    ...
    _respm_hv_f = _np.copy(_api_status.get_respm()[_api_status.get_device_idx('bpm_xy'),:][:,_api_status.get_device_idx('chcv_f')])
    ...
    _respm_v_f = _np.c_[_respm_v_f,_respm_hv_f[-len(_api_status.get_device_idx('bpm_y')):,-1]]
...
def calc_kick(orbit_full = None, ctype = ''):
    ...
    if ctype.lower() == 'h' or ctype.lower() == 'v' or ctype.lower() == 'h_f' or ctype.lower() == 'v_f':
        if ctype.lower() == 'h':
            orbit = orbit_full[_api_status.get_device_idx('bpm_x')]
            reforbit = get_reforbit('x')
            inv_respm = get_inv_respm('h')
        elif ctype.lower() == 'v':
            orbit = orbit_full[_api_status.get_device_idx('bpm_y')]
        ...
        orbit = orbit_full[_api_status.get_device_idx('bpm_xy')]
        if ctype.lower() == 'hv':
