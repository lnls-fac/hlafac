...
def calc_emit(model,
        ...
        short=True):
    """Calculate beam emittance and rms error estimate from quadrupole value
    and measured beam size values.
    
    The input parameters are the model, quadrupole index in model, vector of
    quadrupole values and measured beam sizes. If the measured beam size error
    is not supplied, the error estimate returns None; if given a value, then
    a number of sample experiments will be simulated adding normal errors to
    the beam size propagated by the model and the returned estimate will be
    the standard deviation of the calculated emittances.

    If short is set to False, the model matrix elements are not summed in the
    calculation of the initial beam matrix elements.
    """

    emit, sigma_0 = _calc_emit_from_beam_size(
        model,
        quadrupole_idx,
        k,
        measured_beam_size,
    ...
    if size_error is not None:
        ...
        for i in range(num_samples):
            beam_size_with_errors = propagate_beam_size(
                ...
                k,
                size_error)
            new_emit, new_sigma_0 = _calc_emit_from_beam_size(
                model,
                quadrupole_idx,
                k,
...
def _calc_sigma_0_with_sum(r, sigma_out_11):
    ...
    sigma_0 = _numpy.array([[sigma_vec[0], sigma_vec[1]],
...
def _calc_sigma_0_without_sum(r, sigma_out_11):
    ...
    r_inv = v.transpose().dot(_numpy.diag(1/s)).dot(u.transpose())
